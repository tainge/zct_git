...
md %IPaddress%
cd %IPaddress%
echo 1.系统信息(CreatedbyG) > %IPaddress%
	systeminfo >>　%IPaddress%
echo 2.网卡信息(CreatedbyG) >> %IPaddress%
	ipconfig >>　%IPaddress%
echo 3.监听端口(CreatedbyG) >> %IPaddress%
	netstat -an | find "LISTENING" >> %IPaddress%
echo 4.系统服务(CreatedbyG) >> %IPaddress%
	net start >> %IPaddress%
echo 5.系统进程(CreatedbyG) >> %IPaddress%
	tasklist >> %IPaddress%
echo 6.软件列表(CreatedbyG) >> %IPaddress%
...
echo 7.本地策略(CreatedbyG) >> %IPaddress%
    ...
    del C:\temp.txt
echo 8.系统用户(CreatedbyG) >> %IPaddress%
    net user >> %IPaddress%
...
echo 9.其它选项(CreatedbyG) >> %IPaddress%	
